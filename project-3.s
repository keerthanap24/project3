...
main:
        ...
        jal length_str                  # calls subprogram to find length of string with whitespaces
        addi $s1, $v0, 0                # stores string length in $s0 (saved register)
        ...
        beqz $s1, error			        # branch to error if length of input string ($s1) is 0 exit program
	    bgt $s1, 4, error			    # if length of input string ($s1) is greater than 4 exit program
        ...
        loop_0:						    # loop through the string including whitespaces
                lb $t1, 0($a0)          # loads the first index of $a0 (input string) to temporary register $t1
                beqz $t1, return	    # check if first character is null, if it is branch to return
                addi $a0, $a0, 1		# increment pointer by 1
                addi $t0, $t0, 1		# increment loop by 1
                j loop_0				# return to beginning of loop_0
        return:
                move $v0, $t0			# copies the count (value of $t0) into the value register $v0 
                addi $v0, $v0, -1		# subtract excess from the value register
                move $v1, $a0			# store pointer to last character
                addi $v1, $v1, -2       # subtracts excess from $v1
                jr $ra                  # jump to the return address after loop
        ...
        addi $sp, $sp, -4               # allocates 4 bits in the stack
        sw $s0, 4($sp)                  # pushes the input value into the stack so sub_a can access it via the stack
        jal sub_a                       # calls subprogram A


        sub_a:
                addi $sp, $sp, -4       # allocates 4 bits in the stack
                sw $ra, 0($sp)          # pushing the return address into the stack so $ra can be used by sub_b without disrupting sub_a's return address
                loop:
                    lw $t0, 8($sp)          
                    jal sub_b
